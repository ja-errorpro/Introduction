...
class Actor(cocos.sprite.Sprite):
    ...
    def move(self, offset): #移動處理
        self.position += offset
        self.cshape.center += offset

    def update(self, elapsed): #會重複執行的程式
        pass

    def collide(self, other): #碰撞處理
        pass

class PlayerCannon(Actor):
    KEYS_PRESSED = defaultdict(int)

    def __init__(self, x, y): #初始化的程式
...
class GameLayer(cocos.layer.Layer): #遊戲介面
    is_event_handler = True

    def on_key_press(self, k, _): #判定鍵盤
